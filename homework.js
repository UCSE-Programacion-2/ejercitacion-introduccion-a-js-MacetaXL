...
function acumularPorPasos(inicial, pasos) {
  ...
  for (let i = 0; i < pasos.length; i++) {
    if (pasos[i] >= 0) {
      ...
      total -= Math.abs(pasos[i]); // O simplemente: total += pasos[i] que ya maneja el signo, pero para usar -= explícito:
    }
  }
  return total;
}

// Nota corta para acumularPorPasos: Si la consigna exige usar -= estrictamente para los negativos, se hace restando el valor absoluto. Si solo pide la acumulación, un "total += pasos[i]" resuelve todo, pero así cumple literal con el enunciado.
